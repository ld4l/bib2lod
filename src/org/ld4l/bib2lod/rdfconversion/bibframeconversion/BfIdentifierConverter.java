...
public class BfIdentifierConverter extends BfResourceConverter {
    ...
    private boolean isDuplicateLocalIdentifier(
        ...
        // BfInstanceUriGenerator has created an Identifier object using the
        // URI minted by the LC converter, which is based on the local 
        // identifier as stored in the MARC 001 field. This Identifier is linked
        // to the Instance by the bf:local predicate. This is necessary for 
        // catalogs that record the local identifier only in the 001 field 
        // (Harvard, Stanford). Other catalogs store the local identifier in
        // both the 001 and 035 fields. In this case, a new identifier has been
        // created, as for Harvard and Stanford, so we need to delete the 
        // identifier that the LC converter has created from the 035 field.
        String catalog = Processor.getCatalog(localNamespace);
        if (! catalog.equals("cornell")) {
            return false;
        }
        
        // If the identifier value contains a prefix specifying the scheme, 
        // then it is not a local identifier.
        if (thisPrefix != null) {
            return false;
        }
        ...
        // Find bf:local statements. These are the statements created by 
        // BfInstanceUriGenerator; the LC converter does not generate any 
        // bf:local statements.
        ...
        while (nodes.hasNext()) {
            ...
            // Get the identifier object of the bf:local statement
            Resource identifier = node.asResource();
            if (identifier.equals(subject)) {
                return false;
            }
            
            // Get the value of that identifier
            ...
            // If the current identifier has the same value as the identifier 
            // created by BfInstanceUriGenerator, discard it.
